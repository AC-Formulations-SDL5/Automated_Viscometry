...
# Add src/ to path so we can import the feedback controller
...
try:
    from viscometry.measurement.feedback import RotationalDragFeedbackController, verify_import
    verify_import()  # Confirm successful import
except ImportError as e:
    print(f"❌ Import error: {e}")
    print(f"Searching in path: {src_dir}")
    print(f"Directory exists: {os.path.exists(src_dir)}")
    if os.path.exists(src_dir):
        print(f"Files in directory: {os.listdir(src_dir)}")
    sys.exit(1)
