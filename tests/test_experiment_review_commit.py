...
if str(os.path.join(_ROOT, "src")) not in sys.path:
    sys.path.insert(0, os.path.join(_ROOT, "src"))
from flask import Flask, jsonify

from viscometry.web.app import (
...
class TestExperimentReviewCommit(unittest.TestCase):
    ...
    @patch("viscometry.run.export.save_dynamic_analysis_data")
...
class TestSaveDynamicPartialFilename(unittest.TestCase):
    def test_partial_filename_and_warning_header(self):
        ...
        from viscometry.run.export import save_dynamic_analysis_data

        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            with patch("viscometry.run.export.runs_dir_for_today", return_value=tmp_path):
...
class TestSaveTimeseriesData(unittest.TestCase):
    def test_timeseries_writes_all_samples(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            prev_save_all = run_settings.SAVE_ALL_SAMPLE_DATA
            run_settings.SAVE_ALL_SAMPLE_DATA = True
            try:
                with patch("viscometry.run.export.runs_dir_for_today", return_value=tmp_path):
                ...
                run_settings.SAVE_ALL_SAMPLE_DATA = prev_save_all
...
class TestZStartOffsetRuntime(unittest.TestCase):
    def test_get_safe_z_uses_custom_offset(self):
        from viscometry.calibration.store import get_safe_z_for_cell
        ...
        with patch("viscometry.calibration.store.load_calibration") as mock_load:
