...
TORQUE_TOLERANCE = 5.0             # ±20% tolerance around target
MIN_RPM = 0.1                       # Minimum RPM to test
MAX_RPM = 50.0                     # Maximum RPM to test
RPM_SEARCH_STEP = 0.1               # Initial RPM step size for search
RPM_PRECISION = 0.1                 # Final RPM precision
MAX_SEARCH_ITERATIONS = 30          # Maximum iterations to find target RPM
...
def find_target_rpm_at_z(client: ViscometerClient, z_height: float) -> Tuple[Optional[float], Optional[float], bool, List[Tuple[float, float, float]]]:
    """Find RPM that produces target torque (50% ± 20%) at specific Z-height using binary search approach
    Returns: (final_rpm, final_torque, should_break_cell, all_iterations)
    where all_iterations is a list of (z_height, rpm, torque) tuples for all tested points
    """
    ...
    # Store all iterations: (z_height, rpm, torque)
    all_iterations = []
    ...
    # Record the initial test
    if min_torque is not None:
        all_iterations.append((z_height, MIN_RPM, min_torque))
    ...
    if min_torque is None:
        if ENABLE_RESISTANCE_CHECK:
            ...
            return None, None, False, all_iterations
        else:
        ...
        return None, None, True, all_iterations
        ...
        return MIN_RPM, min_torque, False, all_iterations
    ...
    while iteration < MAX_SEARCH_ITERATIONS and (high_rpm - low_rpm) > RPM_PRECISION:
        ...
        # Record the iteration (even if torque is None)
        if torque is not None:
            all_iterations.append((z_height, mid_rpm, torque))
        ...
        if target_min <= torque <= target_max:
            ...
            return round(mid_rpm, 2), round(torque, 2), False, all_iterations
    ...
    if best_rpm is not None:
        ...
        return round(best_rpm, 2), round(best_torque, 2), False, all_iterations
    else:
        print(f"      NO SOLUTION: Could not find suitable RPM within range {MIN_RPM:.2f} - {MAX_RPM:.2f}")
        return None, None, False, all_iterations

def test_cell_rpm_calibration_z_series(cnc: CNC_Machine, client: ViscometerClient, cell_number: int) -> Tuple[Dict[float, Tuple[Optional[float], Optional[float]]], List[Tuple[float, float, float]]]:
    """Test RPM calibration across Z-gap range for one cell
    Returns: (cell_z_data, all_iterations_data)
    where cell_z_data is {z_height: (rpm, torque)} for final results
    and all_iterations_data is [(z_height, rpm, torque), ...] for all tested points
    """
    print(f"\n{'='*60}")
    ...
    all_iterations_data = []  # [(z_height, rpm, torque), ...]
    current_z = SAFE_Z
    ...
    while current_z >= MAX_Z_TRAVEL or not ENABLE_Z_TRAVEL_LIMIT:
        ...
        try:
            ...
            found_rpm, found_torque, should_break_cell, z_iterations = find_target_rpm_at_z(client, z_rounded)
            cell_z_data[z_rounded] = (found_rpm, found_torque)
            
            # Add all iterations from this Z-height to the collection
            all_iterations_data.extend(z_iterations)
    ...
    try:
        ...
        print("Spindle is coming up")
        time.sleep(5.0)
        cnc.move_to_point(x_pos, y_pos, z=-62.0, speed=25)
        # Stop viscometer once at safe position
        client.stop()
        cnc.move_to_point(x_pos, y_pos, z=0, speed=500)
        time.sleep(2.0)
    ...
    print(f"Total iterations collected: {len(all_iterations_data)} data points")
    return cell_z_data, all_iterations_data

def save_rpm_calibration_data(all_cell_data: Dict[int, Dict[float, Tuple[Optional[float], Optional[float]]]], 
                             all_cell_iterations: Dict[int, List[Tuple[float, float, float]]],
                             timestamp: str) -> List[str]:
    """Save RPM calibration data - one CSV file per cell with all iterations and final results"""
    csv_filenames = []
    ...
    for cell_num in range(1, NUM_CELLS + 1):
        ...
        cell_iterations = all_cell_iterations.get(cell_num, [])
        
        # Create CSV headers for all iterations data
        headers = ["height_cnc", "speed_rpm", "torque_%"]
        ...
        with open(csv_filename, 'w', newline='') as csvfile:
            ...
            # Write all iterations data
            # Sort by Z-height (descending) then by RPM (ascending) for better readability
            sorted_iterations = sorted(cell_iterations, key=lambda x: (-x[0], x[1]))
            
            for z_height, rpm, torque in sorted_iterations:
                row = [
                    f"{z_height:.3f}",
                    f"{rpm:.2f}",
                    f"{torque:.2f}"
...
def main():
    ...
    try:
        ...
        for cell_number in range(1, NUM_CELLS + 1):
            cell_data, cell_iterations = test_cell_rpm_calibration_z_series(cnc, client, cell_number)
            all_cell_data[cell_number] = cell_data
            all_cell_iterations[cell_number] = cell_iterations
            
            print(f"Cell {cell_number} RPM calibration completed")
        
        # Save all data to separate CSV files (one per cell)
        csv_filenames = save_rpm_calibration_data(all_cell_data, all_cell_iterations, timestamp)
