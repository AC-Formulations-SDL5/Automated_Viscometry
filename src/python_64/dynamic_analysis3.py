...
SAFE_Z = -65.9              
Z_STEP_SIZE = -0.0200        #-0.100            
Z_FEED_RATE = 500               
MAX_Z_TRAVEL = -66.500      #-66.700          
...
TEST_RPMS = [0.5] #, 0.5, 1.0, 5.0, 10.0, 20.0, 50.0]
DWELL_SECONDS = 2.0            
INTER_RPM_PAUSE = 2.0           
...
def measure_torque_at_rpm(client: ViscometerClient, rpm: float) -> Optional[List[Dict]]:
    ...
    MEASUREMENT_DURATION = 120.0      #35.0
...
def test_cell_dynamic_z_series(cnc: CNC_Machine, client: ViscometerClient, cell_number: int) -> Dict[float, Dict[float, Optional[List[Dict]]]]:
...
def save_dynamic_analysis_data(all_cell_data: Dict[int, Dict[float, Dict[float, Optional[List[Dict]]]]], 
    ...
    for cell_num in range(1, NUM_CELLS + 1):
        ...
        detailed_filename = f"dynamic_analysis_detailed_cell_{cell_num}_{timestamp}.csv"
        csv_filenames.append(detailed_filename)
        
        # Save summary data (averages for easy analysis)
        summary_filename = f"dynamic_analysis_summary_cell_{cell_num}_{timestamp}.csv"
        ...
        with open(detailed_filename, 'w', newline='') as csvfile:
            ...
            csv_writer.writerow([f"# Dynamic Analysis Detailed - Cell {cell_num}"])
        ...
        with open(summary_filename, 'w', newline='') as csvfile:
            ...
            # Write metadata header
            csv_writer.writerow([f"# Dynamic Analysis Summary - Cell {cell_num}"])
            csv_writer.writerow([f"# Test RPMs: {TEST_RPMS}"])
            csv_writer.writerow([f"# Z-range: {max(z_heights):.3f} to {min(z_heights):.3f}"])
            csv_writer.writerow([f"# Timestamp: {timestamp}"])
            csv_writer.writerow([])
        ...
        print(f"Cell {cell_num} detailed data saved to: {detailed_filename}")
        print(f"Cell {cell_num} summary data saved to: {summary_filename}")
