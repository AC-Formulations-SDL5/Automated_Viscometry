...
SAFE_Z = -64.4              # Min -64.7
Z_STEP_SIZE = -0.05         #-0.100            
Z_FEED_RATE = 500               
MAX_Z_TRAVEL = -65.400      # Max -66.4          
...
# Row configurations: each row has a different BASE_X position
ROWS = [
    {'row_number': 3, 'base_x': 309}
]     #    {'row_number': 1, 'base_x': 10},
      #    {'row_number': 2, 'base_x': 85},
...
def measure_torque_at_rpm(client: ViscometerClient, rpm: float) -> Optional[List[Dict]]:
    ...
    MEASUREMENT_DURATION = 60.0      #35.0
...
def test_cell_dynamic_z_series(cnc: CNC_Machine, client: ViscometerClient, row_number: int, cell_number: int) -> Dict[float, Dict[float, Optional[List[Dict]]]]:
...
def main():
    ...
    for row in ROWS:
        print(f"  Row {row['row_number']}: BASE_X = {row['base_x']}")
    print("="*80)
    ...
    try:
        ...
        for row_config in ROWS:
            ...
            for cell_number in range(1, NUM_CELLS + 1):
                ...
                cell_data = test_cell_dynamic_z_series(cnc, client, row_number, cell_number)
