...
from viscometry.hardware.viscometer.client import ViscometerClient
...
from viscometry.paths import WORKER32_DIR
from viscometry.run.settings import *
import viscometry.run.settings as run_settings
from viscometry.run.cells import global_cell_to_row_and_local, row_and_local_to_global_cell
from viscometry.run.export import (
    save_dynamic_analysis_data,
    save_partial_data,
    maybe_save_timeseries_data,
    is_partial_termination,
    count_z_levels,
    extract_latest_points_from_all_data,
...
def apply_runtime_settings_from_web():
    ...
    rs = run_settings
    web_settings = web_interface.get_runtime_settings()

    rs.EXPERIMENT_NAME = str(web_settings.get('experiment_name', rs.EXPERIMENT_NAME) or '').strip()
    rs.TESTING_MODE = web_settings.get('testing_mode', rs.TESTING_MODE)
    rs.SELECTED_ROWS = web_settings.get('selected_rows', rs.SELECTED_ROWS)
    rs.SELECTED_CELLS = web_settings.get('selected_cells', rs.SELECTED_CELLS)
    rs.TEST_RPMS = web_settings.get('test_rpms', rs.TEST_RPMS)
    raw_map = web_settings.get('cell_rpm_map', {})
    rs.CELL_RPM_MAP = {int(k): v for k, v in raw_map.items() if v}
    raw_content_map = web_settings.get('cell_content_map', {})
    rs.CELL_CONTENT_MAP = {
        int(k): str(v).strip()
        for k, v in raw_content_map.items()
        if str(v).strip()
    }
    rs.Z_STEP_SIZE = float(web_settings.get('z_step_size', rs.Z_STEP_SIZE))
    rs.DWELL_SECONDS = float(web_settings.get('dwell_seconds', rs.DWELL_SECONDS))
    rs.INTER_RPM_PAUSE = float(web_settings.get('inter_rpm_pause', rs.INTER_RPM_PAUSE))
    rs.MEASUREMENT_DURATION = float(web_settings.get('measurement_duration', rs.MEASUREMENT_DURATION))
    rs.SAMPLE_INTERVAL = float(web_settings.get('sample_interval', rs.SAMPLE_INTERVAL))
    rs.SMART_EARLY_EXIT_ENABLED = bool(web_settings.get('smart_early_exit_enabled', rs.SMART_EARLY_EXIT_ENABLED))
    rs.FAIL_SAFE_ENABLED = bool(web_settings.get('fail_safe_enabled', rs.FAIL_SAFE_ENABLED))
    rs.SMART_CV_THRESHOLD = float(web_settings.get('smart_cv_threshold', rs.SMART_CV_THRESHOLD))
    rs.SMART_WINDOW_SIZE = max(2, int(web_settings.get('smart_window_size', rs.SMART_WINDOW_SIZE)))
    rs.FEEDBACK_CONTROL_ENABLED = bool(web_settings.get('feedback_control_enabled', rs.FEEDBACK_CONTROL_ENABLED))
    rs.MIN_DATA_POINTS_FOR_TREND = int(web_settings.get('min_data_points_for_trend', rs.MIN_DATA_POINTS_FOR_TREND))
    rs.R2_DRAG_MIN = float(web_settings.get('r2_drag_min', rs.R2_DRAG_MIN))
    rs.R2_CV_MIN = float(web_settings.get('r2_cv_min', rs.R2_CV_MIN))
    rs.R2_SLOPE_MIN = float(web_settings.get('r2_slope_min', rs.R2_SLOPE_MIN))
    rs.HIT_POINT_CONFIDENCE_THRESHOLD = float(web_settings.get('hit_point_confidence_threshold', rs.HIT_POINT_CONFIDENCE_THRESHOLD))
    rs.TORQUE_BREAK_THRESHOLD = float(web_settings.get('torque_break_threshold', rs.TORQUE_BREAK_THRESHOLD))
    rs.WEIGHT_2ND_DERIV_DRAG = float(web_settings.get('weight_2nd_deriv_drag', rs.WEIGHT_2ND_DERIV_DRAG))
    rs.WEIGHT_2ND_DERIV_CV = float(web_settings.get('weight_2nd_deriv_cv', rs.WEIGHT_2ND_DERIV_CV))
    rs.WEIGHT_2ND_DERIV_SLOPE = float(web_settings.get('weight_2nd_deriv_slope', rs.WEIGHT_2ND_DERIV_SLOPE))
    rs.WEIGHT_R2_DRAG = float(web_settings.get('weight_r2_drag', rs.WEIGHT_R2_DRAG))
    rs.WEIGHT_R2_CV = float(web_settings.get('weight_r2_cv', rs.WEIGHT_R2_CV))
    rs.WEIGHT_R2_SLOPE = float(web_settings.get('weight_r2_slope', rs.WEIGHT_R2_SLOPE))
    rs.BASELINE_N_CALIBRATION = int(web_settings.get('baseline_n_calibration', rs.BASELINE_N_CALIBRATION))
    rs.BASELINE_Z_THRESHOLD = float(web_settings.get('baseline_z_threshold', rs.BASELINE_Z_THRESHOLD))
    rs.CALIBRATION_MODE = bool(web_settings.get('calibration_mode', False))
    rs.RECALIBRATE_INDIVIDUAL_CELLS = bool(web_settings.get('recalibrate_individual_cells', False))
    rs.RECALIBRATION_IGNORE_MAX_Z_TRAVEL = bool(
        web_settings.get('recalibration_ignore_max_z_travel', rs.RECALIBRATION_IGNORE_MAX_Z_TRAVEL)
    )
    rs.LOW_TORQUE_LIQUID_CONTACT_SKIP_ENABLED = bool(
        web_settings.get('low_torque_liquid_contact_skip_enabled', rs.LOW_TORQUE_LIQUID_CONTACT_SKIP_ENABLED)
    )
    rs.LOW_TORQUE_LIQUID_CONTACT_THRESHOLD_PCT = float(
        web_settings.get('low_torque_liquid_contact_threshold_pct', rs.LOW_TORQUE_LIQUID_CONTACT_THRESHOLD_PCT)
    )
    rs.VISCOSITY_PREDICTION_MODE = normalize_viscosity_prediction_mode(
    ...
    try:
        ...
        if not (-1.0 <= z_offset <= 3.0):
            z_offset = 0.4
        rs.Z_START_OFFSET_MM = z_offset
    except (TypeError, ValueError):
        rs.Z_START_OFFSET_MM = 0.4
    rs.DISCOVERY_MODE_ENABLED = bool(web_settings.get('discovery_mode_enabled', False))
    rs.DISCOVERY_PROBE_DURATION_S = float(web_settings.get('discovery_probe_duration_s', rs.DISCOVERY_PROBE_DURATION_S))
    try:
        duck_pct = float(web_settings.get('discovery_duck_torque_pct', rs.DISCOVERY_DUCK_TORQUE_PCT))
        rs.DISCOVERY_DUCK_TORQUE_PCT = max(1.0, min(100.0, duck_pct))
    except (TypeError, ValueError):
        rs.DISCOVERY_DUCK_TORQUE_PCT = 80.0
    try:
        pause_s = float(web_settings.get('discovery_handoff_pause_s', rs.DISCOVERY_HANDOFF_PAUSE_S))
        rs.DISCOVERY_HANDOFF_PAUSE_S = max(0.0, pause_s)
    except (TypeError, ValueError):
        rs.DISCOVERY_HANDOFF_PAUSE_S = 10.0
    raw_eta_map = web_settings.get('discovery_eta_guess_map', {})
    rs.DISCOVERY_ETA_GUESS_MAP = {}
    if isinstance(raw_eta_map, dict):
        for cell_key, eta_val in raw_eta_map.items():
            try:
                ...
                continue
            if eta_val in (None, ''):
                rs.DISCOVERY_ETA_GUESS_MAP[cell_id] = None
            else:
                try:
                    rs.DISCOVERY_ETA_GUESS_MAP[cell_id] = float(eta_val)
                except (TypeError, ValueError):
                    continue
...
def _cell_xy_for_global(global_cell: int) -> Tuple[int, float, float]:
    ...
    _agent_debug_log(
        hypothesis_id="H1",
        location="run/controller.py:_cell_xy_for_global",
...
def _finish_cell_measurement(
    ...
    _agent_debug_log(
        hypothesis_id="H2",
        location="run/controller.py:_finish_cell_measurement",
...
def move_to_cell_position(cnc: CNC_Machine, row_number: int, local_cell_number: int, z_height: float) -> Tuple[float, float, float]:
    ...
    _agent_debug_log(
        hypothesis_id="H1",
        location="run/controller.py:move_to_cell_position",
...
def test_cell_dynamic_z_series(
    ...
    if custom_starting_z is not None:
        ...
        from viscometry.calibration.store import load_calibration
...
def run_predicted_viscosity_for_cell(
    ...
    try:
        from viscometry.rheology.viscosity import predict_cell_viscosity
...
def _merge_discovery_landing_after_descent(
    ...
    try:
        from viscometry.discovery.landing import merge_landing_into_discovery_result
        from viscometry.discovery.mode import load_discovery_config
...
def main():
    ...
    while True:
        ...
        try:
            ...
            worker_path = WORKER32_DIR / "worker32.py"
        ...
        try:
            ...
            for i, global_cell in enumerate(selected_cells):
                ...
                try:
                    ...
                    if DISCOVERY_MODE_ENABLED and not is_calibration_like_run:
                        try:
                            from viscometry.discovery.runner import run_discovery_for_cell
            ...
            try:
                if cnc:
                    ...
                    _agent_debug_log(
                        hypothesis_id="H3",
                        location="run/controller.py:cleanup",
